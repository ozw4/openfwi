...
def main(args):
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if args.output_path and (epoch + 1) % args.epoch_block == 0:
			utils.save_on_master(
				checkpoint, os.path.join(args.output_path, f'model_{epoch + 1}.pth')
			)
...
def parse_args():
	...
	parser.add_argument(
		'-t',
		'--train-anno',
		default='flatfault_b_train_invnet.txt',
		help='name of train anno',
	)
	parser.add_argument(
		'-v',
		'--val-anno',
		default='flatfault_b_val_invnet.txt',
		help='name of val anno',
	)
	parser.add_argument(
		'-o',
		'--output-path',
