...
def evaluate(
	...
	with (vis_path / 'metrics.txt').open('w') as f:
...
def main(cfg: DictConfig):
	...
	target = cfg.val_anno.split('/')[-1][:-4]
	result_path = Path(output_path) / target
	print(output_path)
	print(target)
	...
	with Path('/workspace/OpenFWI-main/dataset_config.json').open() as f:
		try:
